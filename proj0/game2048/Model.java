...
public class Model extends Observable {
  ...
  public boolean tilt(Side side) {
    ...
    for (int col = 0; col < board.size(); col++) {
      boolean[] mergedThisColumn = new boolean[board.size()];  // Track merged status per row
      for (int row = board.size() - 2; row >= 0; row--) {  // Start from second-to-last row upwards
        Tile tile = board.tile(col, row);

        if (tile != null) {
          int targetRow = row;
          for (int i = row + 1; i < board.size(); i++) {
            Tile aboveTile = board.tile(col, i);
            if (aboveTile == null) {  // Empty space, keep moving up
              targetRow = i;
            } else if (aboveTile.value() == tile.value()
                && !mergedThisColumn[i]) {  // Merge condition
              targetRow = i;
              mergedThisColumn[i] = true;  // Mark this row as merged
              getscore += tile.value() * 2;  // Increase score
              break;
            } else {  // Blocked by a tile with a different value
              break;
            }
          }

          if (targetRow != row) {  // Only move if target position has changed
  ...
  public static boolean atLeastOneMoveExists(Board b) {
    ...
    if (b.tile(b.size() - 1, b.size() - 1).value() == b.tile(b.size() - 1, b.size() - 2).value()) {
      return true;
    }
    return false;
  }
